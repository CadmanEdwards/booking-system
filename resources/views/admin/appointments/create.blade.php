    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="row">
                <div class="col-xs-12 form-group">
                    ...
                    <input type="hidden" name="date" id="date" value="{{ isset($_GET['date']) ? $_GET['date'] : date('Y-m-d') }}"/>
                    <input type="hidden" name="selectedTime" id="selectedTime" value="{{ isset($_GET['selectedTime']) ? date('H:i',strtotime($_GET['selectedTime'].' -2 hour')): "" }}"/>
                    <input type="hidden"    id="roomIDInput" value="{{ isset($_GET['roomId']) ? $_GET['roomId'] : "" }}"/>
        ...
        <p>
            @if(isset($_GET['therapistId']))
                <a href="{{ \Illuminate\Support\Facades\URL::to('admin/employees_leaves/create/'.$_GET['therapistId']).'?date='.$_GET['date']  }}"
                   class="btn btn-primary">Set Leaves</a>
                <a href="javascript:void(0)" id="sendToCustomTiming"
                   class="btn btn-primary">Set Custom Timing</a>
            @endif
        ...
        @section('javascript')
            ...
            <script>
                $(document).ready(function() {
                    ...
                    @if(isset($_GET['therapistId']))
                    $('#sendToCustomTiming').on('click',function (){
                        event.preventDefault()
                        var location_id = "{{ $_GET['location_id'] }}";
                        var startTime =  "{{ $_GET['selectedTime'] }}";
                        var url = "{{ \Illuminate\Support\Facades\URL::to('admin/employees_customtiming/create/'.$_GET['therapistId']).'?date='.$_GET['date'] }}";
                        url += "&location_id="+location_id+"&startTime="+startTime;
                        //  console.log(url)
            ...
            <script>
                ...
                $("#client_id").on("change", function() {
                    ...
                    selectedEmployeeDefault('{{ isset($_GET['therapistId']) ? $_GET['therapistId'] : 0 }}')
                });
                    ...
                    if(service_id != "" && date != "" && location_id != "" && employee_id!="") {
                        $.ajax({
                            ...
                            success:function(option){
                                ...
                                $('.borderTimeing').each(function(key,val){
                                    if($.trim($(val).text()) == $('#selectedTime').val()){
                                ...
                                setTimeout(function(){
                                    ...
                                    $('.room_id').each(function(key,val){
                                        if($.trim($(val).val()) == $('#roomIDInput').val()){
                ...
                function selectedEmployeeDefault(employee_id)
                {
                    ...
                    setTimeout(function(){
                        //alert("hello");
                        console.log("selectedEmployeeDefault")

                        $('.employee_id').each(function (key,value) {

                            if($(value).val() == employee_id ){
                                $(value).prop('checked',true).trigger('change')
                            }
                        })

                        // console.log( $('input[name="employee_id"][value="' + employee_id + '"]'))
                        //              $("input[name='employee_id'][value=" + employee_id + "]").prop('checked', true).attr('checked', true).trigger('change');
                        // $(".employee_id").val(option['employee_id']).prop("checked", true);
                    }, 2500);
                }
                function UpdateLocation(client_id)
                {
                    ...
                    if(client_id != "") {
                        $.ajax({
                            ...
                            success:function(option){
                                console.log(option)
                                if(option['have_prev_invoice']){
                                    window.alert("Factuurnummer "+option['have_prev_invoice']['invoice_no']+" van deze cliënt staat open voor "+option['have_prev_invoice']['days']+" dagen. Alleen boeken als deze cliënt gaat pinnen of er voldoende vertrouwen is (uit eerdere betalingen).");
                                }
